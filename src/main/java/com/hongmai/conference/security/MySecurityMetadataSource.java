package com.hongmai.conference.security;
...
public class MySecurityMetadataSource implements SecurityMetadataSource {
     ...
     */
    @Getter
    private static final Set<CpBaseResource> RESOURCES = new HashSet<>();
    ...
    public Collection<ConfigAttribute> getAttributes(Object object) {
        ...
        for (CpBaseResource resource : RESOURCES) {
            ...
            if (request.getMethod().equals(split[0]) && ant.matches(request)) {
                // 将我们权限资源id返回
                return Collections.singletonList(new SecurityConfig(resource.getCode().toString()));
            }
        }
